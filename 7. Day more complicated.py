...
while x == 1:
    ...
    match user_input:
        ...
        case "show" | "display":
            ...
            updated_user_to_do_list = [item.strip('\n') for item in user_to_do_list]

            if updated_user_to_do_list:
                print("The list currently contains: ")
                for number, item in enumerate(updated_user_to_do_list):
        ...
        case "remove" | "delete" | "complete":
            ...
            user_to_do_list = file.readlines()
            file.close()
            # updated_user_to_do_list tylko na potrzeby funkcji print()
            updated_user_to_do_list = [item.strip('\n') for item in user_to_do_list]
            # powrót do pracy na user_to_do_list
            for number, item in enumerate(updated_user_to_do_list):
                row = f"{number + 1}. {item.capitalize()}"
                print(row)
